...
def simplify_docstring(docstr):
    ...
    strip_self = re.sub('self:\s+libsdfTools[^\s\)]+\s+', '', docstr)
    strip_lib  = re.sub('libsdfTools\.', '', strip_self)
    return strip_lib
...
genmodule('sdfTools', mock_name + '/__init__.py', False)


submodules = inspect.getmembers(sys.modules['sdfTools'],
                                lambda member: inspect.ismodule(member)
                                and 'sdfTools' in member.__name__ )

for mname, m in submodules:
    subpath = mock_name + '/' + mname
    pathlib.Path(subpath).mkdir(parents=True, exist_ok=True)
    genmodule('libsdfTools.' + mname, subpath + '/__init__.py', True)
